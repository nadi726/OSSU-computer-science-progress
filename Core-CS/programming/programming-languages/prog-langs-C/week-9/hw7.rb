...
class GeometryValue 
  ...
  def preprocess_prog
    self # no pre-processing for all values except LineSegment
  end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    seg1_points = [x1, y1, x2, y2]
    seg2_points = [seg.x1, seg.y1, seg.x2, seg.y2]
    x1start, y1start, x1end, y1end = seg1_points
	  x2start, y2start, x2end, y2end = seg2_points

		if real_close(x1start, x1end)
      # the segments are on a vertical line
		  # let segment a start at or below start of segment b
      if y1start < y2start
        (aXstart, aYstart, aXend, aYend), (bXstart, bYstart, bXend, bYend) = seg1_points, seg2_points
      else
        (aXstart, aYstart, aXend, aYend), (bXstart, bYstart, bXend, bYend) = seg2_points, seg1_points
      end
