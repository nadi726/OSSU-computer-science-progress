...
;; The function is a more general, or more abstract,
;; than the specifc expressions
(define (area r)
  (* pi (sqr r)))
...
(check-expect (map2 string-length (list "ABC" "12")) (list 3 2))
...
;; Given lon, produce a new list that contains only elements
;; for which (fn element) is true 
(check-expect (filter2 positive? empty) empty)
(check-expect (filter2 positive? (list 1 -2 3 -4)) (list 1 3))
(check-expect (filter2 positive? (list 1 2 3 4)) (list 1 2 3 4))
(check-expect (filter2 positive? (list -1 -2 -3 -4)) empty)
